...
int main (int argc, char **argv) {
    ...
    kafkaConf.SetConf("group.id", "kafka-cpp");
    //kafkaConf.SetConf("auto.offset.reset", "earliest");

    

    if (Kafka::RunConsumer("kafka-cpp-producer", kafkaConf) == -1) {
        return 1;
    }
    std::clog << "consumer work ended" << std::endl;
